...
def detect_card_contour_pokia(frame):
    """
    Détection de carte utilisant la MÊME méthode que pokia.py
    ...
    # Seuillage adaptatif (comme dans pokia.py)
    ...
    for cnt in contours:
        ...
        if area < 5000:  # Ignorer les petits contours
            continue
    ...
    if not contours_filtres:
        # Méthode alternative avec Canny
    ...
    # Trouver le contour le plus rectangulaire (ratio 6.3/8.8)
    ratio_cible = 6.3 / 8.8
...
def draw_pokia_analysis_overlay(frame, contour, points):
    ...
    cv2.putText(overlay, f"Taille: {w}x{h}px", (10, 45), 
